...
describe Game do
  ...
  let(:computer) { double :computer, :move => :Scissors, :pick_move => "", :name => "Computer" }
  subject { Game.new(player, computer) }

  context '#initialize' do
    it 'creates a new game with player vs computer' do
  ...
  context '#message' do
    it 'returns a win message' do
      expect(subject.message).to eq "Player wins!"
    end

    it 'returns a lose message' do
      allow(player).to receive(:move) { :Rock }
      allow(computer).to receive(:move) { :Paper }
      expect(subject.message).to eq "Computer wins!"
    end
